...
自分は試していないので、興味があるほうは公式ドキュメントを参照してください。
...
タップするたびに乱数が生成され、サービスへのログインに使用できます。
...
Yubikeyでpgp鍵を登録する方法として、すでにある鍵を登録する方法と、Yubikeyに生成させて公開鍵だけPCに返す方法があります。
