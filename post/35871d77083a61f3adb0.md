...
Bitcoin自体は、P2Pで動作するアプリケーションです。そのため、通信プロトコルさえあっていれば、PCで動作するBitcoinのソフト（ノード）が違う言語でも、違う作者のものでも正常に動作します。
...
- bitcoin-tx： JSON-RPCに接続し、トランザクションやブロックチェインの情報を取得できるコマンド
- test_bitcoin： JSON-RPCに接続し、サンプルのブロックチェインを渡して、ブラックボックステストが行える
- bench_bitcoin： JSON-RPCに接続し、ノードのベンチマークが行える
- libbitcoinconsensus.h： ブロックチェインの状態に関するライブラリ
...
bitcoin-qtやbitcoindが、ブロックチェインやトランザクションをローカルにファイルとして保存するときに使う、`LevelDB`というkey-value型のデータベースがあり、そのコードが入っています。
...
何をしているかさっぱりわかりませんでした、わかるほうがいれば、教えていただけると嬉しいです。

## test

他の関数に関する、ユニットテストのコードが入っています。base64や、Script、ハッシュ関数など、細部までテストが書かれています。
...
- chainparams.cpp : チェインのパラメーターを実装しています。BIP有効化のブロック高、tcp通信の判別記号、初期ノードのDNSアドレス、ジェネシスブロックのデータ、アドレスのprefixまで、コインに関するほとんどの定数がここで指定されています。regtest、testnet、mainetの3つの関数があります
- key.cpp : 秘密鍵・公開鍵の導出をしています
- pow.cpp : PoWのターゲットの計算、検証を実装しています

# contrib

コントリビューション、つまり、開発をするときに使えるツールや、共同開発をするうえで必要なツール、ビルドツールなどが詰まっています。
