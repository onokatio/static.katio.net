...
23行目の000000e0がおそらくビンゴです。次の`$offset = $text.IndexOf($offset_text) + 0x37`がよくわかりません、が、ここで`Patches/Voltage Patch/bin`以下にcpusetup_my.txtとcpusetup_patched.txtを見つけました。おそらく製作者の方のものでしょう。diffを取ります。
